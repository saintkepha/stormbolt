...
(function() {
  ...
  BoltStream = (function(_super) {
    ...
    function BoltStream(id, stream, config) {
      var cstream;
      this.id = id;
      this.stream = stream;
      this.config = config;
      ...
      this.stream.setKeepAlive(true, 60 * 1000);
      this.forwardingPorts = this.config.allowedPorts;
      ...
      this.mux.on('connection', (function(_this) {
        return function(_stream) {
          return _this.handleAction(_stream, _this.forwardingPorts);
        };
      })(this));
    ...
    BoltStream.prototype.handleAction = function(_stream, _forwardingPorts) {
      var action, breply, bsent, incoming, request, target, _ref, _ref1, _ref2, _ref3;
      _ref = _stream.meta.split(':'), action = _ref[0], target = _ref[1];
      this.log("bolt-mux-connection: action=" + action + " target=" + target);
      _stream.on('error', (function(_this) {
        return function(err) {
          return _this.log(("bolt-mux-connection: mux stream " + _stream.meta + " encountered error:") + err);
        };
      })(this));
      switch (action) {
        case 'capability':
          this.log('sending capability information...');
          _stream.write((_ref1 = this.config) != null ? (_ref2 = _ref1.allowedPorts) != null ? _ref2.join(',') : void 0 : void 0);
          return _stream.end();
        case 'beacon':
          _ref3 = [0, 0], bsent = _ref3[0], breply = _ref3[1];
          _stream.on('data', (function(_this) {
            return function(data) {
              breply++;
              return _this.log("received beacon reply: " + data);
            };
          })(this));
          this.log('sending beacons...');
          return async.whilst((function(_this) {
            return function() {
              var _ref4;
              return bsent - breply < ((_ref4 = _this.config) != null ? _ref4.beaconRetry : void 0);
            };
          })(this), (function(_this) {
            return function(repeat) {
              ...
              bsent++;
              return setTimeout(repeat, ((_ref4 = _this.config) != null ? _ref4.beaconInterval : void 0) * 1000);
            };
          })(this), (function(_this) {
            return function(err) {
              if (err == null) {
        ...
        case 'relay':
          ...
          if (__indexOf.call(_forwardingPorts, target) < 0) {
            this.log("request for relay to unsupported target port: " + target);
            _stream.end();
            break;
          }
          incoming = '';
          ...
          return _stream.on('end', (function(_this) {
            return function() {
        ...
        default:
          this.log("unsupported action/target supplied by mux connection: " + action + "/" + target);
          return _stream.end();
      }
    };
  ...
  StormBolt = (function(_super) {
          ...
          if ((_this.config.uplinks != null) && _this.config.uplinks.length > 0) {
            ...
            async.forever(function(next) {
              ...
              return async.until(function() {
                ...
                return _this.connect(host, port, {
                  key: _this.config.key,
                  cert: _this.config.cert,
                  ca: _this.config.ca,
                  requestCert: true
                }, function(bolt) {
                  if (!(bolt instanceof Error)) {
                    bolt.once('ready', function() {
                      _this.connected = true;
                      retries = 0;
                      bolt.once('close', function(err) {
                        _this.log("bolt.error on " + bolt.id + ":", err);
                        return _this.connected = false;
                      });
                      return bolt.once('error', function(err) {
                        _this.log("bolt.error on " + bolt.id + ":", err);
                        return _this.connected = false;
                      });
                    });
      ...
      server = tls.createServer(options, (function(_this) {
        return function(stream) {
          ...
          try {
            ...
            return typeof callback === "function" ? callback(new BoltStream(cname, stream, _this.config)) : void 0;
          } catch (_error) {
            error = _error;
    ...
    StormBolt.prototype.connect = function(host, port, options, callback) {
      ...
      return stream = tls.connect(port, host, options, (function(_this) {
        return function() {
          var certObj, cname, error, _ref;
          _this.uplink = {
            host: host,
          ...
          try {
            _this.log("TLS connection established with bolt server from: " + stream.remoteAddress);
            certObj = stream.getPeerCertificate();
            cname = certObj.subject.CN;
            _this.log((_ref = ("client connected to " + cname + ": ") + stream.authorized) != null ? _ref : 'unauthorized');
            return typeof callback === "function" ? callback(new BoltStream(cname, stream, _this.config)) : void 0;
          } catch (_error) {
            error = _error;
            _this.log('unable to retrieve peer certificate and authorize connection!', error);
            stream.end();
            return typeof callback === "function" ? callback(new Error("unable to establish bolt connection to server")) : void 0;
          }
        };
      })(this));
    };
