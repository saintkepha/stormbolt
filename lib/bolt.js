...
(function() {
  var BoltRegistry, BoltStream, StormAgent, StormBolt, StormData, StormRegistry,
  ...
  BoltStream = (function(_super) {
    ...
    function BoltStream(id, stream) {
      ...
      this.cstream = this.mux.createReadStream('capability');
      this.cstream.on('data', (function(_this) {
        return function(capa) {
          var _ref;
          _this.log("received capability info from client:", capa);
          _this.capability = (_ref = capa.split(',')) != null ? _ref : [];
          return _this.emit('capability', capa);
        };
      })(this));
      this.bstream = this.mux.createStream('beacon', {
        allowHalfOpen: true
      });
      this.bstream.on('data', (function(_this) {
        return function(beacon) {
          _this.log("received beacon from client: " + _this.id);
          _this.bstream.write("beacon:reply");
          return _this.emit('beacon', beacon);
        };
      })(this));
      ...
      this.stream.on('close', (function(_this) {
        return function() {
          _this.log("bolt stream closed for " + _this.id + " to " + _this.stream.remoteAddress);
    ...
    BoltStream.prototype.relay = function(request, callback) {
      var data, err, firstreply, relay;
      this.log("relay - forwarding request to " + this.id + " at " + this.stream.remoteAddress);
      try {
        ...
        request.on('error', (function(_this) {
          return function(err) {
            _this.log("error processing request via boltstream...", err);
          ...
          return function(chunk) {
            ...
            if (!firstreply) {
              try {
                ...
                err = _error;
                _this.log("invalid first relay response!", err);
                relay.destroy();
              }
              return;
            }
        ...
        relay.on('end', function() {
          return callback(firstreply, data);
        });
        return relay.on('error', function(err) {
          this.log("error during relay multiplexing boltstream...", err);
          return callback(err);
        });
      } catch (_error) {
        err = _error;
        this.log("error duing relaying request to boltstream", err);
        return callback(err);
      }
    };
    ...
    BoltStream.prototype.destroy = function() {
      ...
      try {
        ...
        err = _error;
        return this.log("unable to properly terminate bolt stream: " + bolt.id, err);
      }
    };
  ...
  StormBolt = (function(_super) {
    ...
    StormBolt.prototype.run = function(config) {
      var ca, cacert, chain, connected, err, i, line, res, retries, server, _i, _len, _ref;
      if (config != null) {
      ...
      try {
        ...
        this.activate(null, (function(_this) {
      ...
      if ((this.config.listenPort != null) && this.config.listenPort > 0) {
        server = this.listen(this.config.listenPort, {
          ...
          return function(bolt) {
            if (_this.config.beaconValidity) {
              if (bolt.validity == null) {
                bolt.validity = _this.config.beaconValidity;
              }
            }
            _this.clients.add(bolt.id, bolt);
            bolt.on('beacon', function(beacon) {
              return bolt.validity = _this.config.beaconValidity;
        ...
        this.clients.expires(this.config.repeatdelay);
      }
      if ((this.config.uplinks != null) && this.config.uplinks.length > 0) {
        _ref = [0, 0], i = _ref[0], retries = _ref[1];
        connected = false;
        this.on('client.connection', (function(_this) {
          return function(stream) {
            connected = true;
            return retries = 0;
          };
        })(this));
        this.on('client.disconnect', (function(_this) {
          return function(stream) {
      ...
      if (this.config.allowRelay) {
        return this.proxy(this.config.relayPort);
      }
    };
    ...
    StormBolt.prototype.proxy = function(port) {
      ...
      return acceptor.on("request", (function(_this) {
        return function(request, response) {
          ...
          request.target = port;
          return entry.relay(request, function(reply, body) {
            var err;
