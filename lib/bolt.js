...
(function() {
  ...
  StormBolt = (function(_super) {
    var MuxDemux, async, extend, fs, http, schema, tls, url, util, validate;
    ...
    function StormBolt(config) {
      var ca, cacert, chain, connections, err, line, res, _i, _len;
      if (config != null) {
        this.log('constructor called with:\n' + this.inspect(config));
      }
      res = validate(config, schema);
      this.log('validation of input config: ' + this.inspect(res));
      this.config = require('../package').config;
      if ((config != null) && res.valid) {
      ...
      try {
        ...
        err = _error;
        this.log("unable to retrieve security credentials!");
        throw err;
      }
      this.repeatInterval = 5;
      this.connections = connections = {};
      this.on('server.connect', (function(_this) {
        return function(cname, stream, mx) {
    ...
    StormBolt.prototype.run = function(callback) {
      var connected, i, retries, running, server, _ref;
        ...
        async.whilst((function(_this) {
          ...
          return function(repeat) {
            ...
            _fn = function(key, entry) {
              var err;
              if (!((entry != null) && (entry.mx != null) && (entry.stream != null))) {
                return;
              }
              entry.validity -= _this.repeatInterval;
              if (!(entry.validity > 1)) {
                try {
                  entry.mx.close();
    ...
    StormBolt.prototype.log = function(message) {
      return util.log("" + this.constructor.name + " - " + message);
    };

    StormBolt.prototype.inspect = util.inspect;

    StormBolt.prototype.list = function() {
      var entry, key, _ref, _results;
      this.log('[active bolt connections]');
      _ref = this.connections;
      _results = [];
      for (key in _ref) {
        entry = _ref[key];
        _results.push(this.log("cname: " + key + " allowedPorts: " + entry.allowedPorts + " watchdog: " + entry.watchdog));
      }
      return _results;
    };
    ...
    StormBolt.prototype.listen = function(port, options) {
      ...
      server.on('newconnection', (function(_this) {
        return function(cname, stream) {
          var mx;
          _this.log('connection event triggered ' + _this.inspect(cname));
            ...
            switch (action) {
              ...
              case 'beacon':
                ...
                return async.whilst(function() {
                  ...
                  return _this.log("final call on sending beacons, exiting with: " + (err != null ? err : "no errors"));
                });
              case 'relay':
