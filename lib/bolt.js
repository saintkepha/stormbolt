...
(function() {
  ...
  StormBolt = (function(_super) {
    ...
    StormBolt.prototype.run = function(config) {
      var ca, cacert, chain, connected, err, i, line, res, retries, running, server, _i, _len, _ref;
      if (config != null) {
        this.log('run called with:\n' + this.inspect(config));
        res = validate(config, schema);
        this.log('run - validation of runtime config: ' + this.inspect(res));
        if (res.valid) {
